window.seedResources = async () => {
  ...
  try {
    ...
    for (const item of newSeedData) {
      ...
      if (!catId) {
        console.warn(
          `⚠️ Skipping "${item.title}": Category "${item.cat}" not found.`,
        );
        continue;
      }
      ...
      const payload = {
        ...
        youtube_id: ytId,
        category_id: catId,
        uploader_id: 1, // Explicitly setting admin ID
        program_id: "administration", // Matches the 'text' type in your schema
        institution_id: "upsa",
        is_global: true,
        status: "approved",
      };
