(function () {
          ...
          <div id="collapse${i}" class="accordion-collapse collapse" data-bs-parent="#repDashCoursesSlides">
            <div class="accordion-body p-0">
              <ul class="list-group list-group-flush mb-0">
                ${slides.length === 0 ? `<li class="list-group-item text-muted small">No slides available.</li>` : slides.map(s => `
      ...
      coursesSlides.querySelectorAll('button[data-action]').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          try {
            ...
            showAlert('danger', 'Link expired. Please refresh.');
          }
        });
      });
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    const newTitle = (!newTitleInput.classList.contains('d-none') && newTitleInput.value) ? String(newTitleInput.value).trim() : '';
    const finalCourseTitle = newTitle || selectedTitle;
    const slideTitle = document.getElementById('repDashSlideTitle')?.value.trim();

    if (!finalCourseTitle) return showAlert('danger', 'Please select or enter a course title.');
    if (!slideTitle) return showAlert('danger', 'Please provide a slide title.');
