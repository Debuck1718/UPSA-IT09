document.addEventListener('DOMContentLoaded', async () => {
    ...
    window.processResource = async (id, action) => {
        ...
        try {
            await window.api.fetch(`/api/admin/resources/${id}/${action}`, { method: 'POST' });
    ...
    async function loadAnnouncements() {
        ...
        try {
            ...
            `).join('');
        } catch (e) { container.innerHTML = '<p>No announcements found.</p>'; }
    }
    ...
    document.getElementById('adminLogoutBtn').onclick = () => {
        window.api.fetch('/api/logout', { method: 'POST' }).finally(() => window.location.replace('index.html'));
    };
